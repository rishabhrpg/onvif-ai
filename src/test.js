...
let EVENT_RECEIVER_IP_ADDRESS = '192.168.29.100'; // the IP Address and Port for a HTTP Server that the camera will send events to. Change this.
...
function parseEventXMLManually(xmlBody) {
	...
	try {
		// Look for common patterns in ONVIF event XML
		const topicMatch = xmlBody.match(/<.*?Topic.*?>(.*?)<\/.*?>/);
		const utcTimeMatch = xmlBody.match(/UtcTime="([^"]*)"/);
		const propertyMatch = xmlBody.match(/PropertyOperation="([^"]*)"/);
		
		console.log('📋 PARSED EVENT INFO:');
		if (topicMatch) {
			console.log(`  Topic: ${topicMatch[1]}`);
		}
		if (utcTimeMatch) {
			console.log(`  Time: ${utcTimeMatch[1]}`);
		}
		if (propertyMatch) {
			console.log(`  Property: ${propertyMatch[1]}`);
		}
		
		// Look for motion detection
		if (xmlBody.includes('MOTION') || xmlBody.includes('Motion')) {
			console.log('🚶 MOTION DETECTED!');
		}
		
		// Look for tamper detection
		if (xmlBody.includes('TAMPER') || xmlBody.includes('Tamper')) {
			console.log('⚠️ TAMPER DETECTED!');
		}
		
		// Look for people detection
		if (xmlBody.includes('PEOPLE') || xmlBody.includes('People')) {
		...
		console.log('Error parsing event details:', err.message);
	}
}
	...
	server = http.createServer(function (request, response) {
		...
		request.on('end', () => {
			...
			if (request.method == "POST") {
				...
				if (cam_obj != null) {
					// Parse the XML manually since parseEventXML doesn't exist
					try {
						parseEventXMLManually(body);
					} catch (err) {
						console.log('Error parsing the XML:', err.message);
					}
				}
				return;
			}
